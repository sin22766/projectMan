...
<div use:melt={$portalled}>
	{#if $open}
		...
		>
			...
			<p use:melt={$description} class="mb-2 leading-normal text-sm">
				...
				<slot name='buttonGroup' close={$close}>
					<button
						...
						class="inline-flex h-8 items-center justify-center rounded-md
									px-4 font-medium leading-none border-yellow-800 border"
					>
						Close
					</button>
				</slot>
			...
			<button
				use:melt={$close}
				aria-label="close"
				class="text-yellow-800 hover:bg-yellow-100 focus:shadow-yellow-400 absolute right-4 top-4 inline-flex
