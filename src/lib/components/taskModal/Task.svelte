<script lang="ts">
	...
	export function showModal(task: taskDataInterface) {
        ...
        if (taskData.due_at)
        {
            let date = new Date(taskData.due_at)
		...
		<div class={detailsContainerClasses}>
            ...
            <div class="flex flex-row justify-between">
                <ul class="flex flex-col p-2">
                    <li>
                        Create At: {taskData.created_at ?? '-'}
                    </li>
                    <li>
                        Update At: {taskData.updated_at ?? '-'}
                    </li>
                </ul>
                <div class="flex flex-col p-2">
                    <label>
                        Due at:
                        <input type="datetime-local" value={taskData.updated_at ? taskData.updated_at || null} name="dueDate" autocomplete="off" />
                    </label>
                </div>
